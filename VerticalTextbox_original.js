...
class VerticalTextbox extends fabric.IText {
    ...
    isChangeBoxSize() {
        const fontSizeChanged = this.fontSize !== this.fontSizeOld;
    ...
    isTextChanged() {
        if (this._textOld === undefined) {
            return true;
        }

        const textChanged = this._text.length !== this._textOld.length ||
            this._text.some((char, index) => char !== this._textOld[index]);
    ...
    updateDimensions() {
        ...
        // console.log( "updateDimensions newWidth,  newHeight ", Math.floor(newWidth), Math.floor(newHeight) );

        if (this.width !== newWidth || this.height !== newHeight) {
            if (isTextChanged) {
                if (newWidth > oldWidth) {
                    this.left -= (newWidth - oldWidth);
                } else {
                    this.left += (oldWidth - newWidth);
                }
            } else {
                if (newWidth !== oldWidth) {
                    this.left += (oldWidth - newWidth) / 2;
                }
            }
    ...
    updateDimensions2() {
        ...
        // console.log( "updateDimensions2 newWidth,  newHeight ", Math.floor(newWidth), Math.floor(newHeight) );
    ...
    _wrapText(lines, desiredWidth) {
        // console.trace();
        // console.log("");
        // console.log("\n_wrapText開始: 行数, lines, desiredWidth =", lines.length, JSON.stringify(lines), desiredWidth);
        var wrapped = [], i;
        this.isWrapping = true;
        for (i = 0; i < lines.length; i++) {
            var lineWrapped = this._wrapLine(lines[i], i, desiredWidth);
            //   console.log(`行 ${i + 1} の折り返し結果:`, JSON.stringify(lineWrapped));
            wrapped = wrapped.concat(lineWrapped);
        }
        this.isWrapping = false;

        // console.log("_wrapText終了: 折り返し後の行数, 全体 =", wrapped.length, ":", JSON.stringify(wrapped));
        // console.log("");
        return wrapped;
    }
    _wrapLine(_line, lineIndex, desiredWidth, reservedSpace) {
        // console.log("_wrapLine開始 行, 入力行, 目標幅:",(lineIndex + 1), ":", _line, ":", desiredWidth);
        var lineWidth = 0,
        ...
        // console.log("_wrapLine終了 行, 折り返し結果  :", (lineIndex + 1), ":", JSON.stringify(graphemeLines));
        return graphemeLines;
    }

    // 単語の幅を測定する関数。各文字のグラフェムボックスを取得し、その幅や高さを基にして
    // 総幅を計算する。ラテン文字かどうかを確認し、特殊記号が含まれている場合は高さに
    // ボーナスを追加する。コンソールに詳細な測定ログを出力する。
    ...
    _getLineTopOffset(lineIndex, topOffset) {
        switch (this.verticalAlign) {
            case 'middle':
                return this.getLineWidth(lineIndex) / 2 * -1;
            case 'bottom':
                return topOffset + ((Math.abs(topOffset * 2)) - this.getLineWidth(lineIndex));
            default:
                return topOffset;
        }
    }
    ...
    _isLatin(char) {
        return BRACKETS_REGX.test(char);
    }
    calcTextWidth() {
        let maxLineWidth = 0;
        for (let i = 0, len = this.textLines.length; i < len; i++) {
            const lineWidth = this.getHeightOfLine(i);
    ...
    _getCursorBoundariesOffsets(position) {
        ...
        // console.log("charIndex, lineIndex, getHeightOfLine(i), getWidthOfLine(i)", charIndex, lineIndex, 
        //             this.getHeightOfLine(lineIndex), this.getLineWidth(lineIndex));
        ...
        // console.log("height _getTextHeight():", this.height, ":", this._getTextHeight());

        switch (this.verticalAlign) {
            case 'middle':
                topOffset += (this.height - this.getLineWidth(lineIndex))/2;
                break;
            case 'bottom':
    ...
    renderSelection(boundaries, ctx) {
        ...
        switch (this.verticalAlign) {
            case 'middle':
                initialTopOffset = (this.height - this._getTextHeight()) / 2;
                break;
            case 'bottom':
                initialTopOffset = this.height - this._getTextHeight() - (this.height / 2);
                break;
            default:
                initialTopOffset = boundaries.top;
                break;
        }

        // console.log("verticalAlign top height _getTextHeight():", this.verticalAlign, initialTopOffset, this.height, this._getTextHeight());
        ...
        for (var i = startLine; i <= endLine; i++) {
            ...
            if (i === startLine) {
                if (this.__charBounds && this.__charBounds[startLine] && this.__charBounds[startLine][startChar]) {
    ...
    measureLine(lineIndex) {
        if (!this.__lineWidths) {
            this.__lineWidths = [];
        }
        if (!this.__numberOfSpaces) {
            this.__numberOfSpaces = [];
        }

        if (this.__lineWidths[lineIndex] !== undefined) {
            return {
                width: this.__lineWidths[lineIndex],
                numOfSpaces: this.__numberOfSpaces[lineIndex],
            };
        }
        ...
        for (let i = 0; i < line.length; i++) {
            const char = line[i];
            if (!this.__charBounds[lineIndex][i]) {
                charBox = this._getGraphemeBox(char, lineIndex, i, line[i - 1]);
                this.__charBounds[lineIndex][i] = charBox;
            } else {
                charBox = this.__charBounds[lineIndex][i];
            }
    ...
    _getTextHeight(includeComposition = true) {
        // const isComposing = this.__inputType === 'insertCompositionText' || 
        // this.__inputType === 'deleteCompositionText' ||
        // this.__inputType === 'insertFromComposition';
        // console.log('高さ計算 __compositionData, includeComposition, isComposing    :', this.__compositionData, includeComposition, isComposing);

        let lineIndex = Math.min(this.selectionStart, this.textLines.length - 1);

        let textHeight = 0;
        for (let i = 0, len = this.textLines.length; i < len; i++) {
            const lineWidth = this.measureLine(i).width;
            
            var rotatedHeight = 0;
            if( lineIndex === i && (this.__compositionData) ){
                rotatedHeight = this._getRotatedLineHeight(this.textLines[i]+this.__compositionData, lineWidth);
                // console.log("高さ計算：textLines, textLines+compo, lineWidth, rotatedHeight:", this.textLines[i], this.textLines[i]+this.__compositionData, lineWidth, rotatedHeight);
            }else{
                rotatedHeight = this._getRotatedLineHeight(this.textLines[i], lineWidth);
                // console.log("高さ計算：textLines, lineWidth, rotatedHeight:", this.textLines[i], lineWidth, rotatedHeight);
            }
    ...
    _getRotatedLineHeight(line, lineWidth) {
        let rotatedHeight = lineWidth;
        const charWidth = this._measureWord(line.split(''), 0, 0);
        rotatedHeight = Math.max(rotatedHeight, charWidth);

        // console.log("_getRotatedLineHeight charWidth rotatedHeight lineWidth", charWidth, rotatedHeight, lineWidth );
    ...
    changeWidthAndHeight = wrapWithFireEvent('resizing',
        wrapWithFixedAnchor((_, transform, x, y) => {
            ...
            let target = transform.target, localPoint = getLocalPoint(transform, transform.originX,
                transform.originY, x, y),
