...
class VerticalTextbox extends fabric.IText {
    ...
    updateDimensions() {
        ...
        if (isTextChanged) {
            this._updateDimensionsImpl();
        } else {
            this._updateDimensionsImpl();
        }
    }
    _updateDimensionsImpl() {
        if (!this.dynamicSizing) return;
        ...
        // console.log( "_updateDimensionsImpl minHeight, textHeight", Math.floor(this.minHeight), Math.floor(textHeight) )
        // console.log( "_updateDimensionsImpl minWidth,  textWidth ", Math.floor(this.minWidth), Math.floor(textWidth) );
        // console.log( "_updateDimensionsImpl newWidth,  newHeight ", Math.floor(newWidth),      Math.floor(newHeight) );
    ...
    _getTextHeight() {
        // console.log("\n_getTextHeight, this.textLines.length", this.textLines.length);
        let textHeight = 0;

        for (let i = 0, len = this.textLines.length; i < len; i++) {
            const lineWidth = this.measureLine(i).width;
            const rotatedHeight = this._getRotatedLineHeight(this.textLines[i], lineWidth);
            //   console.log("lineWidth, rotatedHeight:", lineWidth, rotatedHeight);
    ...
    _getRotatedLineHeight(line, lineWidth) {
        ...
        const latinChars = line.match(LATIN_CHARS_REGX);
        const numericChars = line.match(NUMBERIC_REGX);

        if (latinChars || numericChars) {
            const rotatedChars = (latinChars ? latinChars[0] : '') + (numericChars ? numericChars[0] : '');
            const charWidth = this._measureWord(rotatedChars.split(''), 0, 0);
